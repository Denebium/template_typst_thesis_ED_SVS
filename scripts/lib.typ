...
#let school-color-recto = rgb("#E4FF7A")
#let school-color-verso = rgb("#C8D200")
...
#let cover(
  ...
  set text(fill: black)
  text(size: 1.5em, smallcaps[L'UNIVERSITE DE RENNES])
  v(.01cm)
  text(size: 1.2em)[
    #smallcaps[École Doctorale N° 637] \
    _Sciences de la Vie et de la Santé_ \
  ...
  context {
    let y-start = locate(<cover:title>).position().y - .5cm
  ...
  text(size: 1.6em)[*#title*<cover:title>]
  parbreak()
...
#let SVS-thesis(
  ...
  cover(
    ...
    subtitle: draft-string + subtitle,
    author: author,
    ...
    header: context {
      ...
      if calc.odd(i) {
        ...
        if chapter-stack != () {
          ...
          text(0.35cm)[Chapitre #nb -- _ #title _]
          //chapter-stack.first()
        }
      }
  ...
  show heading.where(level: 1): it => {
    ...
    if it.numbering != none {
      context text(
        smallcaps[Chapitre #counter(heading).get().first() \ ],
        size: .45cm,
        weight: "regular",
        //font: "New Computer Modern",
      )
